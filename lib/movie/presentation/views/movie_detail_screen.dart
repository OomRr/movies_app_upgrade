...
class MovieDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
      sl<MovieDetailsBloc>()
        ..add(GetMovieDetailsEvent(id))..add(GetMovieRecommendationsEvent(id)),
      lazy: false,
      ...
      builder: (context, state) {
        switch(state.movieDetailsState) {
          ...
          case RequestState.isError:
            return Text(state.movieDetailsErrorMessage!);
...
/*  Widget _showRecommendations() {
  ...
  builder: (context, state) {
    switch(state.movieRecommendationState) {
      case RequestState.isLoading:
        ...
        return SliverGrid(
          delegate: SliverChildBuilderDelegate(
                (context, index) {
              final recommendation = state.movieRecommendationEntitys[index];
              return FadeInUp(
                from: 20,
                duration: const Duration(milliseconds: 500),
                child: InkWell(
      ...
      case RequestState.isError:
        return Text(state.movieRecommendationErrorMessage!);
    }
